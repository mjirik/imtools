...
def resize3D(data, scale=None, shape=None, sliceId=2, method='cv2'):
    ...
    if data.ndim == 2:
        if shape is not None:
            new_data = cv2.resize_selected_labels(data.astype(np.uint8), shape, 0, 0, interpolation=cv2.INTER_NEAREST)
        elif method == 'cv2':
            if data.dtype == np.bool:
                data = data.astype(np.uint8)
            new_data = cv2.resize_selected_labels(data, (0, 0), fx=scale, fy=scale, interpolation=cv2.INTER_NEAREST)
        else:
        ...
        if sliceId == 2:
            ...
            for i in range(n_slices):
                ...
                if method == 'cv2':
                    new_data[:,:,i] = cv2.resize_selected_labels(data[:, :, i], (0, 0), fx=scale, fy=scale, interpolation=cv2.INTER_NEAREST)
                else:
            ...
            if method == 'cv2':
                if data.dtype == np.bool:
                    data = data.astype(np.uint8)
                new_shape = cv2.resize_selected_labels(data[0, :, :], (0, 0), fx=scale, fy=scale, interpolation=cv2.INTER_NEAREST).shape
            else:
                new_shape =  scindiint.zoom(data[0,:,:], scale).shape
...
def resize_ND(data, scale=None, shape=None, slice_id=0, method='cv2'):
    ...
    if scale is not None:
        new_slice_shape = cv2.resize_selected_labels(data[0, ...], (0, 0), fx=scale, fy=scale, interpolation=cv2.INTER_NEAREST).shape
    else:
    ...
    for i, im in enumerate(data):
        if scale is not None:
            new_data[i, ...] = cv2.resize_selected_labels(im, (0, 0), fx=scale, fy=scale, interpolation=cv2.INTER_NEAREST)
        elif shape is not None:
            new_data[i, ...] = cv2.resize_selected_labels(im, (shape[2], shape[1]), interpolation=cv2.INTER_NEAREST)
...
def resize(image, width=None, height=None, inter=None):
    ...
    # resize the image
    resized = cv2.resize_selected_labels(image, dim, interpolation=inter)
...
def initialize_graycom(data_in, slice=None, distances=(1, ), scale=0.5, angles=(0, np.pi / 4, np.pi / 2, 3 * np.pi / 4),
    ...
    if scale != 1:
        ...
        if blob.ndim == 2:
            blob = cv2.resize_selected_labels(blob, data_in.shape[::-1])
        else:
            tmp = np.zeros(data_in.shape)
            for i, im in enumerate(blob):
                tmp[i, :, :] = cv2.resize_selected_labels(im, (data_in.shape[2], data_in.shape[1]))
            blob = tmp
